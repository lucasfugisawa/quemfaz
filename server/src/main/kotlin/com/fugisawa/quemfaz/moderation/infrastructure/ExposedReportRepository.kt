...
class ExposedReportRepository : ReportRepository {
    ...
    override fun findById(id: ReportId): ServerReport? =
        transaction {
            ReportsTable
                .selectAll()
                .where { ReportsTable.id eq id.value }
                .map { mapReport(it) }
                .singleOrNull()
        }

    override fun list(status: ReportStatus?): List<ServerReport> =
        transaction {
            val query =
                if (status != null) {
    ...
    private fun mapReport(row: ResultRow) =
        ServerReport(
