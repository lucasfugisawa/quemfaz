...
import com.fugisawa.quemfaz.favorites.domain.*
...
class FavoriteServicesTest {
    ...
    private class FakeFavoriteRepository : FavoriteRepository {
        ...
        override fun add(favorite: Favorite) { favorites.add(favorite) }
        override fun remove(userId: UserId, professionalProfileId: ProfessionalProfileId) {
            favorites.removeIf { it.userId == userId && it.professionalProfileId == professionalProfileId }
        }
        override fun exists(userId: UserId, professionalProfileId: ProfessionalProfileId) =
    ...
    private class FakeProfessionalProfileRepository : ProfessionalProfileRepository {
        ...
        override fun listPublishedByCity(cityName: String) = profiles.values.filter { it.cityName == cityName && it.status == ProfessionalProfileStatus.PUBLISHED }
    ...
    fun `should remove favorite`() {
        ...
        val userId = UserId("user-1")
        val profileId = ProfessionalProfileId("prof-1")
        favoriteRepo.add(Favorite(com.fugisawa.quemfaz.core.id.FavoriteId("f1"), userId, profileId, Instant.now()))
    ...
    private fun createProfile(id: ProfessionalProfileId, userId: UserId, status: ProfessionalProfileStatus) = ProfessionalProfile(
        id, userId, "Desc", "Desc", "123", "123", "City",
        emptyList(), emptyList(), emptyList(), ProfileCompleteness.COMPLETE,
        status, Instant.now(), Instant.now(), Instant.now()
    )
}
