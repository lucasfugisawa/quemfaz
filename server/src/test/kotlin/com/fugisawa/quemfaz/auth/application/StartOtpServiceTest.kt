...
class StartOtpServiceTest {
    ...
    private class FakeOtpChallengeRepository : OtpChallengeRepository {
        ...
        override fun create(challenge: OtpChallenge): OtpChallenge { this.challenge = challenge; return challenge }
        override fun findLatestActiveByPhoneNumber(phoneNumber: String): OtpChallenge? = null
        override fun markConsumed(id: String, consumedAt: Instant): Boolean = false
    ...
    @Test
    fun `should start otp correctly`() = runBlocking {
        val otpRepo = FakeOtpChallengeRepository()
        val smsSender = FakeSmsSender()
        val service = StartOtpService(
            otpRepo,
            object : OtpCodeGenerator { override fun generate() = "123456" },
            object : OtpHasher {
                override fun hash(otpCode: String) = "hashed_$otpCode"
                override fun verify(otpCode: String, hash: String) = false
            },
            object : OtpMessageComposer { override fun compose(code: String) = "Code is $code" },
            smsSender,
            OtpConfig(6, 5)
        )
