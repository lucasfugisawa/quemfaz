...
class AdminCatalogIntegrationTest : BaseIntegrationTest() {
    ...
    private fun insertPendingService(
        id: String,
        displayName: String,
        categoryId: String = "CLEANING",
    ) {
        transaction {
    ...
    @Test
    fun `GET pending returns pending services`() =
        adminTestApplication {
    ...
    @Test
    fun `POST approve changes service to active`() =
        adminTestApplication {
            val token = createJwt(ADMIN_USER_ID)
    ...
    @Test
    fun `POST reject changes service to rejected`() =
        adminTestApplication {
            val token = createJwt(ADMIN_USER_ID)
            ...
            try {
                val client = createTestClient(token)
                val response =
                    client.post("/admin/catalog/test-reject-1/reject") {
    ...
    @Test
    fun `POST merge migrates service and profiles`() =
        adminTestApplication {
            val token = createJwt(ADMIN_USER_ID)
            ...
            try {
                val client = createTestClient(token)
                val response =
                    client.post("/admin/catalog/test-merge-source/merge") {
                        contentType(ContentType.Application.Json)
