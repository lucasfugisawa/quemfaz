...
class UxImprovementsIntegrationTest : BaseIntegrationTest() {
    ...
    @Test
    fun `POST auth profile with full name should return 200`() = integrationTestApplication {
    ...
    @Test
    fun `POST auth profile with single word should return 400`() = integrationTestApplication {
    ...
    @Test
    fun `POST auth profile with three-word name should return 200`() = integrationTestApplication {
    ...
    @Test
    fun `PUT date of birth with valid adult date should return 200`() = integrationTestApplication {
        val token = obtainAuthToken("+5511999990020")
        completeNameStep(token, "Maria da Silva")
        val client = createTestClient(token)

        val response = client.put("/auth/me/date-of-birth") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PUT date of birth with underage date should return 422`() = integrationTestApplication {
    ...
    @Test
    fun `GET auth me should return dateOfBirth after setting it`() = integrationTestApplication {
    ...
    @Test
    fun `GET popular services should return 200`() = integrationTestApplication {
    ...
    @Test
    fun `GET popular services with no events should return empty list`() = integrationTestApplication {
        val client = createTestClient()
    ...
    @Test
    fun `GET professional profile should include phone from user account`() = integrationTestApplication {
    ...
    @Test
    fun `profile confirmation without date of birth should fail`() = integrationTestApplication {
        ...
        val confirmResponse = client.post("/professional-profile/confirm") {
            contentType(ContentType.Application.Json)
            setBody("""{"description":"Pintor residencial","selectedServiceIds":[],"cityName":"São Paulo","portfolioPhotoUrls":[]}""")
        }
    ...
    @Test
    fun `profile confirmation with 18+ user should succeed`() = integrationTestApplication {
